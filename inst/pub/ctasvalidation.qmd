...
In practice extreme values in the data set will not only chance one but many time series features. Thus there is a good change that anomalies in the data will create multiple signals that complement each other. CTAS does not try to preemptively select the most appropriate statistical test for a given measurement type and time series length. Therefore all timeseries features should always be measured across all timeseries to cast a wide net for detecting anomalies. 
