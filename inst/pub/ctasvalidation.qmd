...
-   Autocorrelation was introduced by adding the preceeding value multiplied by the anomoly degree to each value.
...
ggplot(df_filt_exp, aes(x = timepoint_rank, y = result, group = subject_id)) +
  ...
  geom_line(data = df_anomaly, aes(y = result_cap_NA), color = "tomato", alpha = 0.5) +
  geom_point(data = df_anomaly, aes(y = result_cap_max, shape = is_lim), color = "tomato", alpha = 0.5) +
...
p_tpr <- ggplot(df_tpr, aes(x = anomaly_degree, y = value, color = site_scoring_method, group = site_scoring_method)) +
  ...
  geom_point() +
  geom_ribbon(aes(ymin = ymin, ymax = ymax, fill = site_scoring_method), alpha = 0.2) +
  ...
  labs(x = "Log-Scaled Anomaly Degree", y = "Average TPR (+/- SD)", color = "Site Scoring Method", fill = "Site Scoring Method") +
  theme_bw() +
  theme(
...
p_fpr <- ggplot(df_fpr, aes(x = anomaly_degree, y = value, color = site_scoring_method, group = site_scoring_method)) +
  ...
  scale_x_log10(breaks = c(0.01, 0.25, 0.5, 0.75, 1, 5, 10), labels = c("0", "0.25", "0.5", "0.75", "1", "5", "10")) +
  facet_grid(parameter_id ~ feats, scales = "fixed") +
  labs(x = "Log-Scaled Anomaly Degree", y = "Average FPR (+/- SD)", color = "Site Scoring Method", fill = "Site Scoring Method") +
  theme_bw() +
  theme(
    strip.text = element_text(size = 8, margin = margin(t = 5, r = 5, b = 5, l = 5)),
    axis.text = element_text(size = 8),
