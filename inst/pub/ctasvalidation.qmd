...
To evaluate CTAS performance for detecting sites with anomalous time series we selected data from two laboratory (Alanine Aminotransferase, Creatinine) and two vitals measurements (Systolic Blood Pressure, Weight) from a clinical trial data set and reassigned all patients to different sites creating a synthetic data set closely following actual trial outcomes but without any potentially confounding site-specifics. Further we removed data points from unscheduled visits and data from screening failures. In total, 15 completed studies were selected from 3 different IMPALA members in triplicates with comparable patient and site number.
...
We have tested three different scoring methods trying to detect 6 different types of site anomalies that were introduced with varying degrees to 15 study data sets donated by 3 different IMPALA members across 4 different measurements. The resulting average TPR and FPR with their respective standard deviations were plotted in @fig-results which shows that TPR are responsive to the degree of the anomalies that were introduced while FPR remain indifferent.
...
We do not observe strong differences between the 4 different measurments. Some of them are more susceptible to the introduction of the anomalies and tend to result in higher TPR at lower anomaly degrees. This is to be expected as the anomalies use multiplication of the existing measurement that are more sensitive to higher starting values. An exception is the weight metric for which CTAS reaches a TPR of less than 50% when trying to detect unique value outliers.

Autocorrelation and local outlier factor anomalies which are more complex than the other anomalies are also difficult to detect we barely detected half of the outliers that were introduced.
