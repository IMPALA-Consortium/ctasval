...
| lof | Local Outlier Factor. Values around one are inliers while the larger the value, more of an outlier the timeseries is. @breunig_lof_2000 |
...
Sites are flagged based on the bias in their time series feature values versus other sites in the study. The user has three options for flagging the sites: Kolmogorov-Smirnov, mixed effects modelling and the simple average of the feature values.
...
To evaluate CTAS performance for detecting sites with anomalous time series we selected data from two laboratory (Alanine Aminotransferase, Creatinine) and two vitals measurements (Systolic Blood Pressure, Weight) from a clinical trial data set and reassigned all patients to different sites creating a synthetic data set closely following actual trial outcomes and schedules. Further we removed data points from unscheduled visits and data from screening failures. In total, 15 completed studies were selected from 3 different IMPALA members in triplicates with comparable patient and site number.

For each anomaly in scope we applied a specific transformation with a given degree to the measurements of three randomly added sites.

-   Autocorrelation was introduced by adding a sinus signal multiplied by the anomaly degree to the original values.

-   The average was increased by adding the site mean multiplied by the anomaly degree to the original values.

-   The local outlier factor for a site would be increased by transforming the data for each patient by a randomly chosen non-normal distribution. The anomaly degree would determine the ratio of affected patients.

-   To simulate range outliers we added one outlier per patient. The extremity of the outlier is based on the anomaly degree.

-   The standard deviation is increased by adding the site mean multiplied with the anomaly degree to each observations and randomly applying a negative or positive fore-sign.

-   The unique value count per site was increased by replacing a ratio of observations with the first observed value per patient. The ratio would be determined by the anomaly degree.

The anomalies introduced are visualized in @fig-anomalies.
...
We tested three different scoring methods to detect six types of site anomalies introduced at varying degrees across 15 study data sets, donated by three different IMPALA members, and measured using four different parameters. The resulting average True Positive Rate (TPR, also known as Recall) and False Positive Rate (FPR), along with their respective standard deviations, were plotted in @fig-tpr and @fig-fpr. The figure shows that TPR rates generally increase with higher anomaly rates, while the FPR either remains flat or slightly decreases as the degree of anomaly increases.
...
Autocorrelation and local outlier factor anomalies, which are more complex, were also difficult to detect. We barely detected half of the introduced outliers. For the autocorrelation feature, the TPR was very low across all methods. This is likely due to the implementation of the function that generated artificial anomalies, suggesting that the current method of introducing autocorrelation anomalies may not be effective, or that the feature itself is less sensitive to the types of anomalies introduced.

Overall, the Mixed Effects Modeling method emerged as the most robust approach for detecting site-level anomalies, providing a good balance between high TPR and low FPR. The average feature value method, while effective in identifying true positives, was less reliable due to its high false positive rate. The performance of the CTAS tool was consistent across different studies. Feature-specific analysis revealed challenges in detecting anomalies in autocorrelation and unique value counts for weight parameters, suggesting areas for further refinement in the anomaly detection process.
...
Altogether this evaluation of the CTAS algorithm has shown that site-level timeseries anomalies can be detected by comparing summary statistics of individual timeseries from one site against those obtained from all other patients from the study.
...
evaluation_results <- read.delim2("inst/pub/evaluation_results.txt")
...
df_tpr <- df_long %>% filter(metric == "avg_tpr" & metric_sd == "sd_tpr")
df_fpr <- df_long %>% filter(metric == "avg_fpr" & metric_sd == "sd_fpr")

# Convert anomaly_degree to a factor to treat it as categorical
df_tpr$anomaly_degree <- as.factor(df_tpr$anomaly_degree)
df_fpr$anomaly_degree <- as.factor(df_fpr$anomaly_degree)

# Create the TPR plot
p_tpr <- ggplot(df_tpr, aes(x = anomaly_degree, y = value, color = site_scoring_method, group = site_scoring_method)) +
  geom_line(linewidth = 1.2) +
  geom_point() +
  geom_ribbon(aes(ymin = value - sd_value, ymax = value + sd_value, fill = site_scoring_method), alpha = 0.2) +
  facet_grid(parameter_id ~ feats, scales = "fixed") +
  labs(x = "Anomaly Degree", y = "Average TPR", color = "Site Scoring Method", fill = "Site Scoring Method") +
  theme_bw() +
  theme(
    strip.text = element_text(size = 10, margin = margin(t = 5, r = 5, b = 5, l = 5)),
  ...
  ggtitle("Line Plots of TPR by Anomaly Degree")

```

```{r echo=FALSE, fig.width=10, fig.height=9}
#| label: fig-fpr
#| fig-cap: "False positive rates"
...
p_fpr <- ggplot(df_fpr, aes(x = anomaly_degree, y = value, color = site_scoring_method, group = site_scoring_method)) +
  ...
  geom_ribbon(aes(ymin = value - sd_value, ymax = value + sd_value, fill = site_scoring_method), alpha = 0.2) +
  facet_grid(parameter_id ~ feats, scales = "fixed") +
  labs(x = "Anomaly Degree", y = "Average FPR", color = "Site Scoring Method", fill = "Site Scoring Method") +
  theme_bw() +
  theme(
    strip.text = element_text(size = 10, margin = margin(t = 5, r = 5, b = 5, l = 5)),
    axis.text = element_text(size = 8),
  ...
  ggtitle("Line Plots of FPR by Anomaly Degree")
